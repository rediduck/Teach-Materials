...
对于战队机器人的开发来说，我们非常在意能否代码运行的实时性

>   例如：遥控器通过串口传来消息，要去实时的解析，否则会变成高pin战士
>
...
​	**注意**：内核启动前至少要有一个任务被创建，但是呢贴心的CubMX帮我们创建好了，我们还是不用管

<img src="./picture/image-20260115170950831.png" alt="image-20260115170950831" style="zoom:80%;float:left;margin-left: 40px"/>

<img src="./picture/image-20260115171037122.png" alt="image-20260115171037122" style="float:left;margin-left: 30px"/>

​	这个任务的优先级是normal，在任务调度器运行的过程中，至少要有一个任务的存在，建议把这个默认任务的优先级设为最低
...
typedef struct {
  ...
  uint32_t                   cb_size;   ///< 提供TCB内存大小（字节）（不常用），默认为0表示未提供TCB内存（并不是没有任务控制块，而是自动分配）
