...
- `osStatus_t osMutexRelease(osMutexId_t mutex_id)`
  ...
  - `osOK`：成功释放
  - `osErrorResource` ：互斥锁没有成功的拿到过或者目前拿锁的线程不是当前释放锁的线程
  - `osErrorParameter`：互斥锁句柄错误
...
类似于一种高端的标志位组，可以用来对不同线程进行**同步**，一个事件标志有32个独立的位，一个线程可以向这（32-1）位置位，然后另一个线程可以读取这个事件的32位，从而知道要做什么。
...
若多个线程都阻塞再等待**相同**的事件标志位，则当事件触发时按照优先级高低依次唤醒（若前面等待完成的处理**不会**改变事件标志的值）

#### 线程标志

##### 用处

线程标志可以看作是私有化的事件标志，作用更多是为了特定的线程服务，而不像事件标志那样类似于布告板之类的。随着线程的创建而自动创建，指定线程属性的cb_size分配的空间就有线程标志的部分。
...
- `uint32_t osThreadFlagsSet(osThreadId_t thread_id, uint32_t flags)`
  ...
  - `osFlagsErrorResource` ：线程处于无效状态（线程已被终止、删除或者内存块损坏）
  - `osFlagsErrorParameter`：线程句柄无效（传入NULL或者其他一些显然错误的值）或者线程标志的最高位被置位

- `uint32_t osThreadFlagsClear(uint32_t flags)`

  用于清除当前正在运行的线程的线程标志的某些位。
...
- `uint32_t osThreadFlagsWait(uint32_t flags, uint32_t options, uint32_t timeout)`
  ...
  用于阻塞当前任务，直到获取线程标志的某些位。==**不能在中断中使用**==
