...
不存在线程的概念，存在多个执行流，如：主执行流、中断服务函数
...
只要栈空间足够，就可以创建很多个任务（不要很多），不必再把不同机构的代码都放在同一个循环中
...
-   对于正赛这样复杂的、多人维护的项目，一个STM32需要控制多个机构且多个机构可能同时执行自己的控制逻辑，主控板**严禁**使用裸机开发
...
>
>   ​	   通过PID控制电机时，我们不希望PID计算的会有很高延迟，否则电机在开环运行下可能会跑飞
>
>   ​           我们希望通过状态机实时掌握各个机构、各个电机的状态，从而可以将机器的控制牢牢掌握在手里

- 用一个 while 循环+中断来控制复杂的机构会很难维护，最好要多个 while +硬中断+软中断相互合作来实现功能
...
2、FREERTOS 只是让不同任务在宏观层面上（使用）看起来像是不同任务在同时进行，实际上因为只有一个MCU，它**只能一行一行地执行代码**，FRREERTOS通过内核调度器让不同任务在一个大的时间上（比如1s）瓜分时间片（1ms）（大致上看来如此，实际还有抢占更复杂的机制），从而让**宏观层面**上看来在这个大的时间内不同地任务都在执行。
...
-   `osStatus_t osKernelStart(void)`
    ...
    用于启动内核和开始任务调度，如果启动成功就不会有返回值！换句话说就是调用的个函数的文件中下文就不会被执行，控制权已经给到了调度器
...
-   `int32_t osKernelLock(void)`
    ...
    用于锁定调度器，停止所有任务**切换**，硬中断依然会执行。**==不能在中断中使用==**
...
- `struct osThreadAttr_t`用于配置任务的属性
    ...
    typedef struct {
          ...
          uint32_t                 attr_bits;   ///< 任务属性位掩码（不常用），默认以分离模式创建，任务在终止时会自动释放资源
          void                      *cb_mem;    ///< 指向任务控制块内存的指针（不常用），默认动态分配内存
          uint32_t                   cb_size;   ///< 提供TCB内存大小（字节）（不常用），默认为0表示未提供TCB内存
          void                   *stack_mem;    ///< 不指向任务内存的指针，默认从内存池动态分配栈
          uint32_t                stack_size;   ///< 任务栈的大小（常用），如果任务的内存设置过小会产生内存溢出的问题导致程序不能正常运行
          osPriority_t              priority;   ///< 任务的优先级（常用），高优先级的任务处于就绪态时可以打断低优先级的任务
...
-   `osThreadId_t`任务句柄类型，相当于每个任务的身份证
...
-   `enum osThreadState_t`任务的状态<a name = "任务状态"></a>
    ...
    {
     	osThreadInactive = 0,			//该任务已创建但未被主动使用，或已被终止,可能任务被挂起了
      	osThreadReady = 1,				//任务已准备好执行，但目前没有运行
      	osThreadRunning = 2,			//任务正在运行
      	osThreadBlocked = 3,			//该任务当前被阻止（延迟，等待事件或暂停）
        osThreadTerminated = 4,			//该任务被终止并释放其所有资源
     	osThreadError = -1,				//任务引发了错误情况，无法进行调度
     	osThreadReserved = 0x7FFFFFFF	//保留，无意义
    }

-   `enum osPriority`任务的优先等级，是RTOS调度器层面的概念
    ...
    与硬件中断相反，软件层面的任务任务优先级数字**越大**，优先级**越高**
    ...
    -   FreeRTOS任务调度器切换不同任务靠的是**SysTick中断**和**PendSV中断**，也就是前文所说的多重任务切换（定时切换+事件切换）。
...
在任务之间交换信息，类似于FIFO（一个任务向队列放入信息，另一个任务从中拿出信息）
...
- `osStatus_t osMessageQueuePut(osMessageQueueId_t mq_id, const void* msg_ptr, uint8_t msg_prio, uint32_t timeout)`
  ...
  - `timeout`：最大消息等待时间（当队列已满时，当前任务应该等待多久）
    - 0：函数立刻返回（消息丢弃）
...
信号量用于管理和保护对资源的访问，例如使得不超过一定数量的任务或中断服务函数访问一组共享资源。
...
- `osStatus_t osSemaphoreAcquire(osSemaphoreId_t semaphore_id, uint32_t timeout)`
  ...
  - `timeout`：阻塞超时时间（单位：内核滴答）
    ...
    - osWaitForever：调用该函数时在信号量可获取之前会阻塞在这个函数，会把调用这个函数的任务置为阻塞态
...
可以看作是深度为1的信号量，可以用来限制一个资源在一段时间内只被一个任务使用

但是信号量任何任务都可释放，互斥量只有持有者可以释放
...
- `struct osMutexAttr_t`用于配置互斥量的属性
  ...
  - `osMutexRecursive`：递归互斥量，允许同一任务多次获取互斥量，但是获取与释放要匹配，如果获取N次就要相应释放N次。
  - `osMutexPrioInherit`：带优先级继承的互斥量，可以防止优先级反转，当高优先级任务等待同一锁时，会临时提高这个持有锁的低优先级任务，从而保证高优先级任务能及时执行。
  - `osMutexRobust`：健壮互斥量，普通互斥量当持有锁的任务意外终止（崩溃、被杀死）时，普通互斥量会永远保持锁定的状态，导致其他需要持有该锁访问资源的任务就会永久等待（死锁），持有健壮互斥量的任务异常终止时，互斥量会进入不一致状态，下一个尝试获取锁的任务会收到错误码，可以选择”恢复“或”清理“互斥量。
  - `0`或默认：不具有以上属性，普通互斥量。
...
- `osStatus_t osMutexAcquire(osMutexId_t mutex_id, uint32_t timeout)`
  ...
  - `timeout`：等待时间
    - 0：该函数立刻返回
    - osWaitForever：一直等待直到互斥量变得可获取，期间该任务会处于阻塞态
    - 0~osWaitForever：阻塞一定的时间，期间若互斥量可获取则直接返回

  返回值：

  - `osOK`：成功获取了这个互斥量，获取成功后任务会由阻塞变为就绪态
...
- `osStatus_t osMutexRelease(osMutexId_t mutex_id)`
  ...
  - `osOK`：成功释放
  - `osErrorResource` ：互斥锁没有成功的拿到过或者目前拿锁的任务不是当前释放锁的任务
  - `osErrorParameter`：互斥锁句柄错误
...
类似于一种高端的标志位组，可以用来对不同任务进行**同步**，一个事件标志有32个独立的位，一个任务可以向这（32-1）位置位，然后另一个任务可以读取这个事件的32位，从而知道要做什么。

**==但是要注意底部实现如果是FreeRTOS的情况只有低24位可用==**

##### 数据结构

- `struct osEventFlagsId_t`用于配置事件标志的属性

  ```c
  typedef struct {
    const char                   *name;   ///< 事件标志名称
    uint32_t                 attr_bits;   ///< 保留位，必须置0
    void                      *cb_mem;    ///< 控制块内存地址，默认或NULL为动态分配
...
- `osEventFlagsId_t osEventFlagsNew(const osEventFlagsAttr_t* attr)`
  ...
  传入参数：事件标志属性，NULL表示默认

  返回值：事件标志句柄

- `uin32_t osEventFlagsSet(osEventFlagsId_t ef_id, uint32_t flags)`

  用于设置事件标志的某些位。==**可以在中断中使用**==

  传入参数：

  - `ef_id`：事件标志句柄
  - `flags`：指定哪些位要设置，**注意**最高位是错误的标志位，**==不要设置==**

  可能返回：

  - 设置后的事件标志值
  - `osFlagsErrorUnknown`：发生未知错误
  - `osFlagsErrorResource`：目标事件标志处于不可使用状态
  - `osFlagsErrorParameter`：目标事件标志的句柄不存在或者事件标志的错误位已经被置1

- `uin32_t osEventFlagsClear(osEventFlagsId_t ef_id, uint32_t flags)`

  用于清除事件标志的某些位。==**可以在中断中使用**==
  ...
  - `osFlagsErrorUnknown`：发生未知错误 
  - `osFlagsErrorResource`： 目标事件标志处于不可使用状态
  - `osFlagsErrorParameter`：目标事件标志的句柄不存在或者事件标志的最高位已经被置1

- `uint32_t osEventFlagsWait(osEventFlagsId_t ef_id, uint32_t flags, uint32_t options, uint32_t timeout)`

  用于等待获取事件标志的某些位。==**如果timeout设0可以在中断中使用**==
  ...
  - `options`：等待模式选项
    ...
    - `osFlagsWaitAny`：`flags`指定的任何一位有效时，函数将此位置置0并返回，否则阻塞
    - `osFlagsWaitAll`：`flags`指定的所有位置都有效时，函数将所有指定的位置置0并返回，否则阻塞
    - `osFlagsNoClear`：和前两种模式搭配使用，只是不会改变事件标志中的值

    若组合使用在模式间加`|`即可，例如：`osFlagsWaitAll|osFlagsNoClear`

  - `timeout`：等待时间

    - 0：函数立刻返回
    - osWaitForever：会一直阻塞除非满足等待完成标准
    - 0~osWaitForever：若在指定阻塞时间到之前满足条件则立刻返回，否则阻塞一定时间再返回

  返回值：

  - 清除前事件标志值
  - `osFlagsErrorUnknown`：发生未知错误
  - `osFlagsErrorTimeout`： 在预期的时间预期的标志位没有被置1
  - `osFlagsErrorResource`： timeout为0时，不满足目标位置置位要求
  - `osFlagsErrorParameter`：目标事件标志的句柄不存在或者事件标志的最高位已经被置1

若多个任务都阻塞再等待**相同**的事件标志位，则当事件触发时按照优先级高低依次唤醒（若前面等待完成的处理**不会**改变事件标志的值）

#### 任务标志

##### 用处

任务标志可以看作是私有化的事件标志，作用更多是为了特定的任务服务，而不像事件标志那样类似于布告板之类的。随着任务的创建而自动创建，指定任务属性的cb_size分配的空间就有任务标志的部分。
...
- `uint32_t osThreadFlagsSet(osThreadId_t thread_id, uint32_t flags)`
  ...
  - `thread_id`：任务句柄
  - `flags`：要设置的位，**注意最高位不要设置**

  可能返回：

  - 设置后的任务通知值。
  - `osFlagsErrorUnknown`：未知错误 
  - `osFlagsErrorResource` ：任务处于无效状态（任务已被终止、删除或者内存块损坏）
  - `osFlagsErrorParameter`：任务句柄无效（传入NULL或者其他一些显然错误的值）或者任务标志的最高位被置位

- `uint32_t osThreadFlagsClear(uint32_t flags)`

  用于清除当前正在运行的任务的任务标志的某些位。

  参数1为要清除的位。==**不能在中断中使用**==

  传入参数：要清除的位，**注意最高位不能置1**

  可能返回：

  - 清除前的任务通知值。
  - `osFlagsErrorUnknown`：未知错误
  -  `osFlagsErrorISR` ：函数错误地从中断服务程序中调用
  - `osFlagsErrorParameter`：传入的要清除的位中最高位被**错误**地置1

- `uint32_t osThreadFlagsWait(uint32_t flags, uint32_t options, uint32_t timeout)`

  用于阻塞当前任务，直到获取任务标志的某些位。==**不能在中断中使用**==

  传入参数：

  - `flags`：要获取的位，**注意最高位不能置1**
  - `options`：等待模式选项
    - `osFlagsWaitAny`：`flags`指定的任何一位有效时，函数将此位置置0并返回，否则阻塞
    - `osFlagsWaitAll`：`flags`指定的所有位置都有效时，函数将所有指定的位置置0并返回，否则阻塞
    - `osFlagsNoClear`：和前两种模式搭配使用，只是不会改变事件标志中的值
  ...
  - 调用此函数前的事件标志值。
  - `osFlagsErrorTimeout` 阻塞超时。
  - `osFlagsErrorUnknown`：未知错误 
  - `osFlagsErrorResource`： timeout为0时，不满足目标位置置位要求
  - `osFlagsErrorParameter`：传入参数`flags`的最高位被错误地置1
...
- `osStatus osDelay(uint32_t ticks)`（相对延时）**==不能在中断中使用==**
  ...
  运行时相当于在这个函数中延长指定的时间，此时内核并不是不运行了，在一个任务的osDelay期间，这个任务是处于阻塞态的，此时MCU会去运行别的就绪态的任务，等阻塞了指定的时间会把这个任务置为就绪态。
...
- os家族与HAL_Dealy区别
  ...
  用`osDelay`或者`osDelayUntil`时在延时的过程中是不消耗资源的，且如果调用这个函数的任务优先级比较高，在阻塞期间可以把资源释放出来给低优先级任务运行。
  
  用`HAL_Delay`时并不会把掉哟的调用该函数的任务挂入阻塞态，如果有其他同优先级的任务，就会在任务调度器分给自己的那个时间片内一直运行HAL_Delay,用完分得的时间片。如果其他就绪态任务优先级都比自己低，那么该任务就会一直运行，从而让低优先级任务饿死。
  
  而且当多个任务都用HAL_Delay时，可能会对延时的准确性造成影响。所以如果用CMSIS-RTOS2尽量都使用`os`系列，十分滴好用。

#### 软件定时器

<img src="./pictures/image-20260121154131881.png" alt="image-20260121154131881" style="margin-left: 0px;"/>

软件定时器并不是由硬件外设直接产生中断，而是由 **RTOS 的系统节拍中断（Tick）驱动实现的逻辑定时机制**。

RTOS 会在每一次系统 Tick 中断中更新系统时间，并检查当前是否有软件定时器到期。当某个软件定时器到期时，内核并不会在中断中直接执行回调函数，而是向一个专门的**定时器服务任务（Timer Service Task）**发送事件，由该任务在普通任务上下文中执行对应的回调函数。

一般来说F407板上TIM资源对于正赛赛说是完全足够的，因为舵机很少会用到，且用的数量不会那么大，优先推荐大家使用硬件定时器，不过软件定时器操作起来比较灵活。

##### 数据结构

- `struct osTimerAttr_t`用于配置定时器属性
...
- `void (*osTimerFunc_t)(void* argument)`
  ...
  定时器回调函数原型，**不要放延时函数！**用CMSIS-RTOS2的API也最好用中断中可以使用的函数
...
- `osTimerId_t osTimerNew(osTimerFunc_t func, osTimerType_t type, void* argument, const osTimerAttr_t* attr)`
  ...
  - `argument`：如果创建的定时器回调函数需要有外部参数传入，则在此传入
  - `attr`：定时器属性
  
- `osStatus_t osTimerStart(osTimerId_t timer_id, uint32_t ticks)`

  用于启动或重新启动目标定时器。==**不能在中断中使用**==

  传入参数：

  - `timer_id`：定时器句柄

  - `ticks`：指定的定时器周期（单位：系统滴答）

  返回值：

  - `osOK`：成功启动定时器或者重新启动定时器
  -  `osErrorISR`：错误地从中断服务程序中调用此函数
  - `osErrorParameter` 目标定时器地址不存在或周期小于等于0
  - `osErrorResource` 目标定时器处于无效状态。
...
- `osStatus_t osTimerStop(osTimerId_t timer_id)`
  ...
  -  `osErrorParameter` ：定时器句柄错误
  - `osErrorResource`：定时器已经停止
  - `osErrorISR`：错误地从中断服务程序中调用该函数

#### 总结

至此所有常见常用的API接口已经介绍完了，不过在实际使用时还是会有一些细节的地方要注意

### 3、实际使用时一些要注意的地方

多任务系统中存在一种潜在的风险。当一个任务在使用某个资源的过程中，即还没有完全结束对资源的访问时，便被切出运行态，使得资源处于非一致，不完整的状态。如果这个时候有另一个任务或者中断来访问这个资源，则会导致数据损坏或是其它相似的错误。

例如使用电机的时候，底盘有四个电机，我们不希望用任务对四个电机获取值时当获取到了第二个电机的时候就被切出了运行态，然后下个时间片恰巧又是对这四个值进行解算的过程，此时这四个值在时间维度上就不统一，用起来可能就会出一些奇怪的问题。

所以当我们进行一些资源写入或获取时可以短暂地关闭任务调度器，让当前任务写入或获取完再切换到下一个任务，**甚至**关闭中断。

可以用互斥量进行解决，下面会介绍一种另外的解决方式

##### 基本临界区

在CMSIS-RTOS2中没有让任务进入临界区的函数，因此我们只能借助FreeRTOS的原生函数

可以使用`taskENTER_CRITICAL()`和`taskEXIT_CRITICAL()`指定临界区，当进入临界区后，优先级低的中断会被屏蔽，且任务调度器不会进行任务的调度。
...
todo的东西是一口气完成的，切记里面不可以塞延时函数！一方面是为了实时性，另一方面是如果放了osDelay之类的，此时优先级为15的systick中断已经被屏蔽了，就会卡死在osDelay中，==切忌切忌==。此外**不要在临界区中使用CMSIS-RTOS2的API**

至于哪些中断会被屏蔽呢，CubMX中可以进行设置，所有优先级低于（指的是优先级不是数值）和等于`LIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY`的中断都会被屏蔽掉

<img src="./pictures/image-20260121212908925.png" alt="image-20260121212908925" style="margin-left: 0px;"/>

可以看到CubMX在使用FreeRTOS时会自动地把CAN、TIM、USART等中断优先级设为5，FreeRTOS可屏蔽的界限也是5

<img src="./pictures/image-20260121214416232.png" alt="image-20260121214416232" style="margin-left: 0px;zoom: 80%;"/>
...
<img src="./pictures/image-20260121231832967.png" alt="image-20260121231832967" style="margin-left: 0px;zoom: 80%;"/>

2、增加FreeRTOS中间层

<img src="./pictures/image-20260121231950964.png" alt="image-20260121231950964" />

其他就无了，可以自行看看战队仓库借鉴我们是怎么用多线程构建工程的
